...
int _dsp_cycle(int * weights, int size, int src, int dst) {
	char * visited = new char[size];
	int * dist = new int[size];
	memset(dist, 0xFF, size*8);
	memset(visited, false, size);
	...
	while(!visited[dst]) {
		...
		visited[mindist] = true;
		for(int i = 0; i < size; i++) {
			if(weights[mindist*size+i]!=0xFFFFFFFFFFFFFFFF and dist[i]>-1) {
				return 1;
			}
			if(weights[mindist*size+i]!=0xFFFFFFFFFFFFFFFF and (dist[i] > dist[mindist]+weights[mindist*size+i] || dist[i]==-1)) {
				dist[i] = dist[mindist]+weights[mindist*size+i];
...
int * _size_map(int * list, int size) {
	int power = 1;
	int s = 1 << (1+_bin_size(size));
	int * ret = new int[2*s];
	memset(ret,0,s);
	memcpy(ret+s,list,size*8);
	while(s > 1) {
		s /= 2;
		for(int i = s; i < s*2; i++) {
			ret[i] = MAX(ret[i*2],ret[i*2+1]);
		}
	}
	ret[0] = size;
	return ret;
}
int _max_in_range(int * list, int start, int end) {
	int upper_bound = end + (1 << (_bin_size(list[0])+1));
	int lower_bound = start + (1 << (_bin_size(list[0])+1));
	int highest = 0;
	while(upper_bound!=lower_bound) {
		highest = MAX(MAX(list[upper_bound],list[lower_bound]),highest);
		upper_bound = (upper_bound-1) >> 1;
		lower_bound = (lower_bound+1) >> 1;
	}
	return MAX(list[upper_bound],highest);
}
